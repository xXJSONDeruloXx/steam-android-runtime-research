...
PROBE="$SCRIPT_DIR/device/nova-termux-x11-rootfs-dev-bind-probe.sh"
RUNTIME_CLEANUP="$SCRIPT_DIR/device/nova-runtime-cleanup.sh"
MOUNT_PRIVATE=${NOVA_MOUNT_PRIVATE_HELPER:-$BUILD_DIR/nova-mount-private}
RUN_ID=${NOVA_RUN_ID:-termux-x11-$(date -u +%Y%m%dT%H%M%SZ)-dev-bind-probe-0}
RUN_DIR=${NOVA_RUN_DIR:-$BUILD_DIR/manual-runs/$RUN_ID}
REMOTE_PROBE=/data/local/tmp/nova-termux-x11-rootfs-dev-bind-probe-$RUN_ID.sh
...
if [ ! -x "$MOUNT_PRIVATE" ] || ! command -v sha256sum >/dev/null 2>&1; then
    echo "missing mount-private helper or host sha256sum" >&2
    exit 1
fi

mkdir -p "$RUN_DIR"
for artifact in \
    run-metadata.txt nova-termux-x11-rootfs-dev-bind-probe.sha256 \
    nova-runtime-cleanup.sha256 nova-mount-private.sha256 \
    pre-cleanup.txt probe-output.txt post-cleanup.txt; do
...
on_exit() {
    ...
    if [ "$RUN_STARTED" -eq 1 ]; then
        ...
        adb shell su -c "/system/bin/rm -f $REMOTE_PROBE $REMOTE_CLEANUP $REMOTE_MOUNT_PRIVATE" >/dev/null 2>&1 || status=1
    fi
    exit "$status"
}
...
adb push "$MOUNT_PRIVATE" "$REMOTE_MOUNT_PRIVATE" >/dev/null
adb shell "chmod 755 $REMOTE_PROBE $REMOTE_CLEANUP $REMOTE_MOUNT_PRIVATE"
...

{
    ...
    echo "mount_private=$MOUNT_PRIVATE"
    echo "mount_private_sha256=$(sha256sum "$MOUNT_PRIVATE" | awk '{print $1}')"
...
sha256sum "$PROBE" >"$RUN_DIR/nova-termux-x11-rootfs-dev-bind-probe.sha256"
sha256sum "$RUNTIME_CLEANUP" >"$RUN_DIR/nova-runtime-cleanup.sha256"
sha256sum "$MOUNT_PRIVATE" >"$RUN_DIR/nova-mount-private.sha256"

RUN_STARTED=1
adb shell su -c "/system/bin/sh $REMOTE_PROBE $DEVICE_ROOT $REMOTE_MOUNT_PRIVATE" \
    >"$RUN_DIR/probe-output.txt" 2>&1
echo "termux_x11_dev_bind_probe=pass run_id=$RUN_ID"
