...
CONTROL="$SCRIPT_DIR/device/gamescope-headless-wayland-shm-control.sh"
...
DEVICE_CONTROL="$DEVICE_DRIVER_DIR/gamescope-headless-wayland-shm-control.sh"
CONTROL_REPORT="$BUILD_DIR/device-gamescope-headless-composite-report.txt"
...
"$ADB" push "$CONTROL" "$DEVICE_STAGE/gamescope-headless-wayland-shm-control.sh" >/dev/null
"$ADB" shell "su -c 'cp $DEVICE_STAGE/gamescope-headless $DEVICE_BINARY; cp $DEVICE_STAGE/wayland-shm-control $DEVICE_CLIENT; cp $DEVICE_STAGE/gamescope-headless-wayland-shm-control.sh $DEVICE_CONTROL; chmod 755 $DEVICE_BINARY $DEVICE_CLIENT $DEVICE_CONTROL'"

set +e
VULKAN_ICD_FILE=/opt/nova-kgsl-driver/freedreno-kgsl.icd.json \
VULKAN_NODEVICE_SELECT=1 \
VULKAN_OFFSCREEN_PROBE=/opt/nova-kgsl-driver/gamescope-headless-wayland-shm-control.sh \
...
if [ "$probe_status" -ne 0 ]; then
    echo "headless gamescope composite probe failed; inspect $CONTROL_REPORT" >&2
    exit "$probe_status"
fi
...
expected_markers=(
    ...
    'wayland_shm_frames=[1-9]'
    'headless_composite_frame='
...
echo "headless_gamescope_composite=pass"
