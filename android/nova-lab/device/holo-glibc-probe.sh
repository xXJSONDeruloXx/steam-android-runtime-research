...
if [ "\$status" -eq 0 ]; then
    run_holo_env() {
        if [ -n "\$VULKAN_ICD_FILE" ]; then
            ...
            /system/bin/chroot "\$ROOT" /usr/bin/env -i PATH=/usr/bin:/bin HOME=/tmp VK_LOADER_DEBUG="\$VULKAN_LOADER_DEBUG" NODEVICE_SELECT="\$VULKAN_NODEVICE_SELECT" NOVA_AHB_ASYNC_FENCE="\$VULKAN_AHB_ASYNC_FENCE" NOVA_AHB_DOUBLE_BUFFER="\$VULKAN_AHB_DOUBLE_BUFFER" NOVA_AHB_OUTPUT_SOCKET="\$VULKAN_AHB_OUTPUT_SOCKET" "\$@"
        fi
    }
