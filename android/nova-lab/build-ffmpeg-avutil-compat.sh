...
docker run --rm --platform linux/arm64 --name nova-ffmpeg-avutil-compat-build \
    -v "$REPO_DIR:/src:ro" \
    ...
    debian:trixie-slim bash -lc '
        ...
        gcc --sysroot=/sysroot -I/usr/include -L/usr/lib \
            ...
            /src/android/nova-lab/device/ffmpeg-avutil-compat.c \
            -ldl \
            -Wl,--version-script=/src/android/nova-lab/ffmpeg-avutil-compat.map \
            -o /out/libffmpeg-avutil-compat.so
        file /out/libffmpeg-avutil-compat.so
    '

echo "$BUILD_DIR/libffmpeg-avutil-compat.so"
