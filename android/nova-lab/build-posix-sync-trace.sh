...
docker run --rm --platform linux/arm64 --name nova-posix-sync-trace-build \
    -v "$REPO_DIR:/src:ro" \
    ...
    debian:trixie-slim bash -lc '
        ...
        gcc --sysroot=/sysroot -I/usr/include -L/usr/lib \
            ...
            /src/android/nova-lab/device/posix-sync-trace.c \
            -ldl -pthread \
            -Wl,--version-script=/src/android/nova-lab/device/posix-sync-trace.map \
            -o /out/libposix-sync-trace.so
        file /out/libposix-sync-trace.so
    '

echo "$BUILD_DIR/libposix-sync-trace.so"
