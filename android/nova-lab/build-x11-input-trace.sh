...
SYSROOT="$BUILD_DIR/x11-input-trace-sysroot"
OUTPUT=${NOVA_X11_INPUT_TRACE_OUTPUT:-$BUILD_DIR/nova-x11-input-trace}
OUTPUT_NAME=$(basename "$OUTPUT")
DOCKER_IMAGE=${NOVA_X11_BUILD_IMAGE:-debian:trixie-slim}

LIBX11_PACKAGE=$(printf '%s\n' "$PACKAGE_DIR"/libx11-*.pkg.tar.zst | head -n 1)
LIBXI_PACKAGE=$(printf '%s\n' "$PACKAGE_DIR"/libxi-*.pkg.tar.zst | head -n 1)
LIBXEXT_PACKAGE=$(printf '%s\n' "$PACKAGE_DIR"/libxext-*.pkg.tar.zst | head -n 1)
LIBXFIXES_PACKAGE=$(printf '%s\n' "$PACKAGE_DIR"/libxfixes-*.pkg.tar.zst | head -n 1)
...
for package_file in "$LIBX11_PACKAGE" "$LIBXI_PACKAGE" \
    "$LIBXEXT_PACKAGE" "$LIBXFIXES_PACKAGE" "$LIBXCB_PACKAGE" \
    "$LIBXAU_PACKAGE" "$LIBXDMCP_PACKAGE" "$XORGPROTO_PACKAGE"; do
    if [ ! -f "$package_file" ]; then
        echo "missing XInput2 build package: $package_file" >&2
        exit 1
    fi
done
...
docker run --rm --platform linux/arm64 \
    -v "$SCRIPT_DIR:/src" \
    -v "$BUILD_DIR:/out" \
    -e "NOVA_X11_INPUT_TRACE_OUTPUT_NAME=$OUTPUT_NAME" \
    "$DOCKER_IMAGE" \
    sh -lc '
set -e
...
SYSROOT=/out/x11-input-trace-sysroot
mkdir -p "$SYSROOT"
tar --zstd -xf /src/build/holo-rootfs/packages/libx11-*.pkg.tar.zst -C "$SYSROOT"
tar --zstd -xf /src/build/holo-rootfs/packages/libxi-*.pkg.tar.zst -C "$SYSROOT"
tar --zstd -xf /src/build/holo-rootfs/packages/libxext-*.pkg.tar.zst -C "$SYSROOT"
tar --zstd -xf /src/build/holo-rootfs/packages/libxfixes-*.pkg.tar.zst -C "$SYSROOT"
...
cp /src/device/nova-xge-compat.h \
    "$SYSROOT/usr/include/X11/extensions/Xge.h"
cc \
    --sysroot="$ROOTFS" \
    ...
    /src/device/nova-x11-input-trace.c \
    -lXi -lXfixes -lXext -lX11 -lxcb -lXau -lXdmcp \
    -o "/out/$NOVA_X11_INPUT_TRACE_OUTPUT_NAME"
file "/out/$NOVA_X11_INPUT_TRACE_OUTPUT_NAME"
'
