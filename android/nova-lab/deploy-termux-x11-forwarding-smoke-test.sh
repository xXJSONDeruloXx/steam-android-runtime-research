...
case "$DBUS_SESSION_USER" in
    root|steam)
        ;;
    *)
        echo "NOVA_TERMUX_X11_DBUS_SESSION_USER must be root or steam" >&2
        exit 2
        ;;
esac
case "$DBUS_SESSION_UID_RECORD" in
    0|1)
        ;;
    *)
        echo "NOVA_TERMUX_X11_DBUS_SESSION_UID_RECORD must be 0 or 1" >&2
        exit 2
        ;;
esac
case "$DBUS_SYSTEM_MODE" in
    0|1)
        ;;
    *)
        echo "NOVA_TERMUX_X11_DBUS_SYSTEM must be 0 or 1" >&2
        exit 2
        ;;
esac
...

{
    ...
    echo "dbus_session_user=$DBUS_SESSION_USER"
    echo "dbus_session_uid_record=$DBUS_SESSION_UID_RECORD"
    echo "dbus_system=$DBUS_SYSTEM_MODE"
