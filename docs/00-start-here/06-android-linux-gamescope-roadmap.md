...
nested client-root/HOME/cwd contract reproduced the same `vgui2_s` fatal. The
next step is a read-only module-loader/environment audit comparing rooted and
rootless path visibility, argv/cwd/HOME, loader paths, dependencies, and the
Steam module-loader boundary. Keep Runtime 4, Proton, and packaging deferred
until fresh evidence identifies the failing layer.
