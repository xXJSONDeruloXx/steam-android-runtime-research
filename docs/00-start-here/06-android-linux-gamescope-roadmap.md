...
step is one smallest justified device A/B run. Do not guess a
...
2. **Close the native ARM64 client lifecycle gate.** R17 and R18b isolated
   ...
   now justifies a one-file Valve `libSDL3.so.0` A/B. Do not patch SteamUI or
   create a guessed `vgui2_s.dll` alias.
