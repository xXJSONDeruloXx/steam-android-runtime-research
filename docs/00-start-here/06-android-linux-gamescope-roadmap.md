...
to change only the sanitized current rooted public tree first. Do not guess a
`.so`-to-`.dll` alias, preload an exploratory adapter, or patch SteamUI.
...
2. **Close the native ARM64 client lifecycle gate.** R17 and R18b isolated
   ...
   rootless contract. Do not patch SteamUI or create a guessed alias.
