...
- Partially answered: can the headless Gamescope seam sustain display-size Android
  AHardwareBuffer output for Steam’s actual frames, not just the current 60-frame 64x64 Wayland
  SHM control? The Nova connector now completes a 30-frame 960x540 `wl_shm` run with 30
  acquire-fence handoffs, 30 SurfaceControl completions, and 29 release-fence returns. Steam’s
  actual Wayland/Xwayland workload remains untested.
